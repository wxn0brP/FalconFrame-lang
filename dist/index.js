...
function translateHtml(html, langData) {
    html = html.replace(/<([^>]*?)\s+translate\s*=\s*["']([^"']+)["']([^>]*?)>(.*?)<\/[^>]*?>/gs, (fullMatch, beforeTranslate, translateValue, afterTranslate, innerHTML) => {
    ...
    html = html.replace(/(<[^>]*?)\s+translate-([a-z-]+)=["']([^"']+)["']([^>]*>)/gi, (fullMatch, beforeTag, attrName, attrValue, afterTag) => {
